...
class PostGameElement(UserElement):
    def __init__(self, rect: Rect, absolute_position: Tuple):
        ...
        self.text = Text(Rect(100, 300, 200, 25), (absolute_position[0], absolute_position[1]))

        self.restart_button = Button(Rect(100, 100, 600, 100), (absolute_position[0], absolute_position[1]))
    ...
    def get_render_info(self, transform: Tuple, buffer_to_render: BufferToRender):
        ...
        self.text.draw_info.text = f"{buffer_to_render.battle_result}! Очки: {buffer_to_render.points}"
